...
export function triggerChaos(room, requestedBy) {
...
export function noteMessageForChaos(room, userId, username, text) {
    const ev = room.chaos;
    if (!ev || ev.awarded.includes(userId))
        return;
    ...
    if (isEmojiRound || isOneWord)
        return;
    const count = ev.participants.filter((id) => id === userId).length;
    if (ev.kind === "RAPID_FIRE" && count >= 3)
        return;
    if (!ev.awarded.includes(userId) && count >= 1 && ev.kind !== "RAPID_FIRE")
        return;
...
export function noteReactionForChaos(room, userId, username) {
    const ev = room.chaos;
    if (!ev || ev.awarded.includes(userId))
        return;
