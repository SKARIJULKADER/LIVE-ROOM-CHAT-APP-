...
export function startMystery(room) {
    if (room.mystery)
        return { error: "A mystery round is already running." };
    const users = [...room.users.values()];
    if (users.length < 3)
        return { error: "Mystery mode needs at least 3 players." };
    const impostorCount = Math.max(1, Math.min(2, Math.floor(users.length / 4)));
    const pool = [...users];
    const impostors = [];
    for (let i = 0; i < impostorCount && pool.length > 0; i++) {
        const idx = Math.floor(Math.random() * pool.length);
        impostors.push(pool.splice(idx, 1)[0].id);
    }
    const now = Date.now();
    const round = {
        roundId: uid("myst-"),
...
function beginVoting(room, round) {
    if (room.mystery?.roundId !== round.roundId || round.revealed)
        return;
    round.phase = "voting";
...
export function castMysteryVote(room, userId, targetId) {
    const round = room.mystery;
    if (!round || round.revealed)
        return { error: "No active mystery round." };
    if (round.phase !== "voting")
        return { error: "Voting hasn't started yet." };
    if (round.votesBy[userId])
        return { error: "You already voted." };
    if (!round.alive.includes(userId))
        return { error: "You are not in this round." };
    if (!round.alive.includes(targetId))
    ...
    if (me2)
        awardXp(room, userId, 5, "mystery_vote", me2.username);
    return {};
}
function revealMystery(room, round) {
    if (room.mystery?.roundId !== round.roundId || round.revealed)
        return;
    round.revealed = true;
    const tally = new Map();
...
export function onUserLeftMystery(room, userId) {
    const round = room.mystery;
    if (!round || round.revealed)
        return;
...
//# sourceMappingURL=mystery.js.map