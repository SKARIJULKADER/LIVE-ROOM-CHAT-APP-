...
let nextSchedule = new Map();
export function startChallenge(room, picks) {
    if (room.challenge)
        return;
...
export function finishChallenge(room, championOverride) {
...
export function noteMessageForChallenge(room, userId, username, text) {
    const ch = room.challenge;
    if (!ch || ch.participants[userId]?.completed)
        return;
...
export function noteReactionForChallenge(room, userId, username) {
    const ch = room.challenge;
    if (!ch || ch.participants[userId]?.completed)
        return;
    if (ch.rule !== "reaction")
...
export function scheduleNext(room) {
...
export function cancelChallengeSchedule(roomId) {
