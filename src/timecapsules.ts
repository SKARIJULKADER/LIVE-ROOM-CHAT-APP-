...
export function createTimeCapsule(
  ...
  if (text.length < 3) return { error: "Capsule note is too short." };
  if (text.length > 500) return { error: "Capsule note is too long (500 chars max)." };

  if (cond.type === "date") {
    const now = Date.now();
    if (cond.at < now + 10_000) return { error: "Unlock time must be at least 10s in the future." };
    if (cond.at > now + 1000 * 60 * 60 * 24 * 90) return { error: "Unlock time is too far away (90 days max)." };
  } else if (cond.type === "messages") {
    if (cond.count < 3 || cond.count > 10000) return { error: "Message target must be between 3 and 10 000." };
  } else {
    return { error: "Invalid unlock condition." };
  }

  const now = Date.now();
  const capsule: ChatMessage = {
    id: uid("cap-"),
    roomId: room.id,
    userId,
    username,
  ...
  if (room.capsules.length > MAX_CAPSULES) room.capsules.length = MAX_CAPSULES;
...
export function tickCapsules(room: Room): void {
  if (room.capsules.length === 0) return;
  const now = Date.now();
  for (const capsule of room.capsules) {
    if (!capsule.locked || !capsule.unlockCondition) continue;
    let due = false;
...
function unlockCapsule(room: Room, capsule: ChatMessage): void {
  ...
  if (creator) awardXp(room, capsule.userId, 10, "capsule_unlocked", creator.username);
  console.log(`[capsule] ${room.id} unlocked -> "${capsule.text.slice(0, 40)}"`);
}

export function startCapsuleTicks(): void {
  setInterval(() => {
