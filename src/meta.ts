...
export function analyzeMessage(room: Room, msg: ChatMessage): void {
  ...
  if (HUMOR_RE.test(msg.text)) room.traits.humor++;
  if (DEEP_RE.test(msg.text)) room.traits.deep++;
  if (POSITIVE_RE.test(msg.text)) room.traits.positivity++;
  if (
    tokens <= 2 ||
  ...
  if (hour >= 22 || hour < 5) room.traits.lateNight++;
...
export function trackReaction(room: Room, emoji: string): void {
...
export function trackMostReacted(room: Room, messageId: string, text: string, count: number): void {
...
// ---------------------------------------------------------------------------
export function computePersonality(room: Room): { score: number; nice: number; title: string } {
  const t = room.traits;
  ...
  const high = (v: number, floor = 55) => v >= floor;
  if (high(lateNight, 45) && high(chaos, 55)) title = "The Midnight Chaos Club";
  else if (high(chaos, 70)) title = "Certified Chaos Theory";
  else if (high(humor, 65)) title = "The Meme Department";
  else if (high(deep, 60)) title = "The Deep Talk Society";
  else if (high(lateNight, 60)) title = "The 2AM Society";
  else if (high(activity, 70)) title = "Professional Procrastinators";
  else if (high(positivity, 65)) title = "The Compliment Corner";
  else if (total < 5) title = "The Quiet Corner";
...
// ---------------------------------------------------------------------------
const STAGES: readonly { min: number; name: string }[] = [
  { min: 0, name: "🌱 Seed" },
  { min: 20, name: "🌿 Sprout" },
  { min: 60, name: "🍃 Sapling" },
  { min: 150, name: "🌳 Tree" },
...
export function computeWorld(room: Room): Room["world"] {
  const s = room.stats;
  const points = Math.round(
    s.messageCount * 1 + s.reactionCount * 0.5 + s.eventCount * 4 + s.challengeCount * 6
  );
  let stage = 0;
...
// ---------------------------------------------------------------------------
export function metaPayload(room: Room): {
  personality: {
    humor: number;
    chaos: number;
    deep: number;
...
export function broadcastMeta(room: Room): void {
  broadcast(room, "ROOM_META_UPDATE", metaPayload(room));
}

export function startMetaTicks(): void {
  setInterval(() => {
