...
import type { Room, RoomUser, PublicUser } from "./types.js";
...
// ---------------------------------------------------------------------------
const rateBuckets = new WeakMap<WebSocket, { count: number; resetAt: number }>();

function rateLimited(ws: WebSocket): boolean {
  const now = Date.now();
...
// ---------------------------------------------------------------------------
function publicUser(u: RoomUser): PublicUser {
  return {
    id: u.id,
    username: u.username,
    status: u.status,
...
function setTyping(room: Room, user: RoomUser, typing: boolean): void {
  if (user.isTyping === typing) return;
...
const typingTimers = new WeakMap<RoomUser, ReturnType<typeof setTimeout>>();

function scheduleTypingClear(room: Room, user: RoomUser): void {
...
// ---------------------------------------------------------------------------
export function buildRoomState(room: Room, userId: string): Record<string, unknown> {
...
function handleFrame(ws: WebSocket, frame: unknown): void {
...
function currentUser(ws: WebSocket): { userId: string; room: Room | undefined } {
  ...
  if (!userId) return { userId: "", room: undefined };
  const roomId = roomOfUser.get(userId);
  const room = roomId ? getRoom(roomId) : undefined;
  return { userId, room };
}
function dispatch(ws: WebSocket, type: string, payload: unknown): void {
  const { userId, room } = currentUser(ws);
  if (!userId && type !== EVT.JOIN_ROOM) return missingJoin(ws);
...
function missingJoin(ws: WebSocket): void {
...
// join / rejoin
// ---------------------------------------------------------------------------
function handleJoin(ws: WebSocket, payload: Record<string, unknown>): void {
...
// messages
// ---------------------------------------------------------------------------
function handleMessage(room: Room, userId: string, payload: Record<string, unknown>): void {
  const user = room.users.get(userId);
  if (!user) return;
  const text = typeof payload.text === "string" ? payload.text.trim().slice(0, MAX_MESSAGE_LEN) : "";
  if (!text) {
    const sock = socketByUser.get(userId);
    if (sock) sendEvent(sock, EVT.ERROR, { code: "EMPTY_MESSAGE", message: "Empty messages can't be sent." });
    return;
  }
...
function activityPush(room: Room, ts: number): void {
  const act = room.activity;
  act.messageTs.push(ts);
  if (act.lastMsgAt > 0) act.gaps.push((ts - act.lastMsgAt) / 1000);
  act.lastMsgAt = ts;
  if (act.messageTs.length > 500) act.messageTs.splice(0, act.messageTs.length - 500);
  if (act.gaps.length > 200) act.gaps.splice(0, act.gaps.length - 200);
}

// ---------------------------------------------------------------------------
// typing / presence
// ---------------------------------------------------------------------------
function handleTyping(room: Room, userId: string, typing: boolean): void {
  const user = room.users.get(userId);
  if (!user) return;
  user.lastActivityAt = Date.now();
  if (typing) {
...
// reactions
// ---------------------------------------------------------------------------
function handleReaction(room: Room, userId: string, payload: Record<string, unknown>): void {
  const user = room.users.get(userId);
  if (!user) return;
  ...
  if (!(ALLOWED_REACTIONS as readonly string[]).includes(emoji)) {
    const sock = socketByUser.get(userId);
    if (sock) sendEvent(sock, EVT.ERROR, { code: "BAD_EMOJI", message: "That reaction isn't allowed." });
    return;
  }
  const message = room.messages.find((m) => m.id === messageId);
  if (!message) {
    const sock = socketByUser.get(userId);
    if (sock) sendEvent(sock, EVT.ERROR, { code: "NOT_FOUND", message: "Message not found." });
    return;
  }
  user.lastActivityAt = Date.now();
  ...
  if (added) {
    ...
    if (message.userId !== userId) applyXp(room, message.userId, 1, "reaction_received");
  } else {
    list.splice(idx, 1);
    message.reactedBy[emoji] = list;
    message.reactions[emoji] = list.length;
...
function reactionTotal(message: { reactions: Record<string, number> }): number {
  let total = 0;
  for (const n of Object.values(message.reactions)) total += n;
  return total;
}

function applyXp(room: Room, userId: string, amount: number, reason: string): void {
  const user = room.users.get(userId);
  if (!user) return;
  user.xp += amount;
...
// ---------------------------------------------------------------------------
function handleChallengeInteract(room: Room, userId: string, payload: Record<string, unknown>): void {
  const user = room.users.get(userId);
  if (!user) return;
  ...
  if (!ch || ch.id !== challengeId) {
    const sock = socketByUser.get(userId);
    if (sock) sendEvent(sock, EVT.ERROR, { code: "NO_CHALLENGE", message: "No matching challenge." });
    return;
  }
  if (ch.participants[userId]) return;
...
// ---------------------------------------------------------------------------
function handlePollCreate(room: Room, userId: string, payload: Record<string, unknown>): void {
  const user = room.users.get(userId);
  if (!user) return;
  ...
  if ("error" in res) {
    const sock = socketByUser.get(userId);
    if (sock) sendEvent(sock, EVT.ERROR, { code: "BAD_POLL", message: res.error });
  }
}

function handlePollVote(room: Room, userId: string, payload: Record<string, unknown>): void {
  ...
  if ("error" in res) {
    const sock = socketByUser.get(userId);
    if (sock) sendEvent(sock, EVT.ERROR, { code: "BAD_VOTE", message: res.error });
  }
}

function handleCapsuleCreate(room: Room, userId: string, payload: Record<string, unknown>): void {
  const user = room.users.get(userId);
  if (!user) return;
  ...
  const condition =
    condType === "messages"
      ? { type: "messages" as const, count: typeof condPayload.count === "number" ? Math.floor(condPayload.count) : 100 }
      : { type: "date" as const, at: typeof condPayload.at === "number" ? condPayload.at : Date.now() + 60_000 };
  const res = createTimeCapsule(room, userId, user.username, { text, condition });
  if ("error" in res) {
    const sock = socketByUser.get(userId);
    if (sock) sendEvent(sock, EVT.ERROR, { code: "BAD_CAPSULE", message: res.error });
  }
}

function handleMysteryActivate(room: Room, userId: string): void {
  const res = startMystery(room);
  if ("error" in res) {
    const sock = socketByUser.get(userId);
    if (sock) sendEvent(sock, EVT.ERROR, { code: "MYSTERY_ERR", message: res.error });
  }
}

function handleMysteryVote(room: Room, userId: string, payload: Record<string, unknown>): void {
  const targetId = typeof payload.targetId === "string" ? payload.targetId : "";
  const res = castMysteryVote(room, userId, targetId);
  if (res.error) {
    const sock = socketByUser.get(userId);
    if (sock) sendEvent(sock, EVT.ERROR, { code: "MYSTERY_VOTE_ERR", message: res.error });
  }
}
...
// ---------------------------------------------------------------------------
export function removeUserFromRoom(userId: string): void {
  const roomId = roomOfUser.get(userId);
  if (!roomId) return;
...
export function startPresenceTicks(): void {
  setInterval(() => {
    const now = Date.now();
    for (const room of rooms.values()) {
      ...
      if (changed) broadcastPresence(room);
...
export function onSocketClose(ws: WebSocket): void {
  ...
  if (userId) removeUserFromRoom(userId);
}