...
export function createPoll(
  ...
  if (opts.length < 2 || opts.length > 4) return { error: "Polls need 2–4 options." };
  ...
  if (room.polls.length > MAX_POLLS) room.polls.length = MAX_POLLS;
...
export function votePoll(room: Room, pollId: string, userId: string, option: number): Poll | { error: string } {
  const poll = room.polls.find((p) => p.id === pollId);
  if (!poll) return { error: "Poll not found." };
  if (poll.closed) return { error: "This poll is closed." };
  if (poll.voters.includes(userId)) return { error: "You already voted." };
  if (!poll.options[option]) return { error: "Invalid option." };
